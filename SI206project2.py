...
def find_urls(s):
    ...
    for word in words:
        ...
        if len(sites) < 1:
            continue
        #If the lists have string in them that match the regular expression then index into the lists
        websites = sites[0]
        l.append(websites)
    return l
...
def grab_headlines():
    ...
    l = list()
    #Find the <div> tag that the Most Read section is under
    ...
    #Find all the links under the Most Read section
    links = div.find_all('a')
    #For each link, get the text, and append it to a list
    for each in links:
...
def get_umsi_data():
    umsi_titles = {}
    #There are thirteen pages in the UMSI directory so were going to iterate 13 times
    count = 13
    #List of just the names text
    all_names = list()
    #List of just the titles text
    all_titles = list()
    #Want my iterator variable to be page numbers so I use the range function
    for number in range(count):
        #The only difference between all the page URLs is the (page =) part at the end so I can use string formatting
        #to have my iterator variable be the page number so each time I iterate the page number changes to the next one
        ...
        html = request.text
        soup = BeautifulSoup(html, "html.parser")
        #The names are located in <div> tags under "parent" <div> tags so I want to get a list of all of those
        listnames = soup.find_all('div', attrs = {'class': 'field-item even', 'property': 'dc:title'})
        #The titles are located in <h2> tags under "parent" <div> tags so I want to get a list of all of those
        listtitles = soup.find_all('div', attrs = {'class': 'field field-name-field-person-titles field-type-text field-label-hidden'})
        #Iterating through my list, I want to extract the title which is located in the string portion of a <div> tag and append it to my list
        for titles in listtitles:
            title = str(titles.div.string)
            all_titles.append(title)
        #Iterating through my list, I want to extract the name which is located in the string portion of a <h2> tag and append it to my list
        for names in listnames:
            name = str(names.h2.string)
            all_names.append(name)
        #Giving all key's in my dictionary empty values
        for each in all_names:
            umsi_titles[each] = ''
        #Creating my dictionary. Zip turns my two lists into a list of tuples, and then dict turns my list of tuples into a dictionary by matching the keys to values
...
def num_students(data):
    ...
    for students in data:
        #If the value if PhD student then increase my counter
